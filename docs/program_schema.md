...
observation (camera only)
  ...
  ground(observation)   the one place coordinates appear
        |
        v
  Plan / SkillStep -> compile_plan -> actions        (legacy verifier, yaw-free only)
  policy segments  -> run_trace_segments -> MuJoCo   (live redetect + bounded retry)
```
...
**Redetection and retry are runtime operations.** The nominal `grounded_trace` records the
initial-scene compilation used for preprocessing. During replay, the policy yields one trace
segment at a time; after a mid-program `detect`, the camera is read again and the remaining
waypoints are rebound before execution continues. `redetect_regrasp` reruns the same bounded
program from a fresh current observation after a missed grasp, not a placement miss. `abort` executes nothing.
...
The existing action-conditioned checkpoint predates yaw and now refuses yaw-bearing traces
instead of silently scoring different orientations as identical. The #18 scorer must encode
yaw explicitly before orientation-dependent ranking is run.
...
| `protocol` | protocol and schema version, git SHA + dirty flag, generator settings/code hash, generation time |
...
a ranking with a gap, candidates out of generation order, missing/short/non-nested prefixes, a
missing or empty grounded trace, a declining candidate that somehow carries a trace, the
wrong schema version or generator hash, a repeated or unlabelled diagnostic, a dropped `pool_has_success`, a
...
The cache keys on the model, prompts, schema/ranges, generation budget, Claude CLI version,
and validation/grounding/dedup implementation hash, so changing candidate semantics
invalidates old pools rather than silently mixing generations. Git SHA and dirty state are
also recorded. Pool size is excluded from the identity: a larger pool of the same kind keeps
the existing ordered samples and issues only the additional calls. The generate command prints each pool as
`cached`, `stale`, or `small` before it decides to spend anything.
Generation refuses tracked worktree edits by default; `--allow-dirty` is only for explicitly
exploratory pools, never the locked run.
